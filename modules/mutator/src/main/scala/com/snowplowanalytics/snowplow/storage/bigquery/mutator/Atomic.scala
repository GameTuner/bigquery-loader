...
import com.snowplowanalytics.iglu.schemaddl.bigquery.{Field, Mode, Type => DataType}
...
object Atomic {
  val table = List(
    ...
    // Data/time
    Field("enricher_tstamp", DataType.Timestamp, Mode.Nullable),
    ...
    Field("geo_country_name", DataType.String, Mode.Nullable),
    ...
    Field("event_tstamp", DataType.Timestamp, Mode.Nullable),
    Field("event_quality", DataType.Integer, Mode.Nullable),
    Field("sandbox_mode", DataType.Boolean, Mode.Nullable),
    Field("backfill_mode", DataType.Boolean, Mode.Nullable),
    Field("date_", DataType.Date, Mode.Nullable)
  )
}
