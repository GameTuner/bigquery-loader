...
import com.snowplowanalytics.snowplow.external.badrows.{BadRow, Failure, FailureDetails, Payload}
      ...
      .map {
        case Right(response) if response.hasErrors =>
          ...
          val failure = errors
            ...
            .getOrElse(
              Failure
                .LoaderRecoveryFailure(FailureDetails.LoaderRecoveryError.RuntimeError(errors.toString(), None, None))
            )
