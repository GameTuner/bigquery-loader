 ...
 */
package com.snowplowanalytics.snowplow.external.analytics.scalasdk.decode

// java
import com.snowplowanalytics.snowplow.external.analytics.scalasdk.validate.FIELD_SIZES
...
// This library
import com.snowplowanalytics.snowplow.external.analytics.scalasdk.Common.{ContextsCriterion, UnstructEventCriterion}
import com.snowplowanalytics.snowplow.external.analytics.scalasdk.SnowplowEvent.{Contexts, UnstructEvent}
import com.snowplowanalytics.snowplow.external.analytics.scalasdk.ParsingError.RowDecodingErrorInfo
import com.snowplowanalytics.snowplow.external.analytics.scalasdk.ParsingError.RowDecodingErrorInfo._
...
private[decode] object ValueDecoder {
  ...
  implicit final val localDateColumnDecoder: ValueDecoder[LocalDate] =
    fromFunc[LocalDate] {
      case (key, value) =>
        if (value.isEmpty) {
          InvalidValue(key, value, s"Field ${key.name} cannot be empty").asLeft
        } else {
          try  LocalDate.parse(value).asRight
          catch {
            case _: DateTimeParseException =>
              InvalidValue(key, value, s"Cannot parse key ${key.name} into LocalDate").asLeft
          }
        }
    }

  implicit final val localDateOptionColumnDecoder: ValueDecoder[Option[LocalDate]] =
    fromFunc[Option[LocalDate]] {
      case (key, value) =>
        if (value.isEmpty) {
          none[LocalDate].asRight[RowDecodingErrorInfo]
        } else {
          try LocalDate.parse(value).some.asRight
          catch {
            case _: DateTimeParseException =>
              InvalidValue(key, value, s"Cannot parse key ${key.name} into LocalDate").asLeft
          }
